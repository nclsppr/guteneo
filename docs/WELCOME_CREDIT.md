...
A D1 organization-insert trigger writes the grant in the same transaction as account creation. Under the Auth0 Free beta policy (`verified_email`), signup requires the validated Auth0 identity, verified email and the signed `https://guteneo.com/verified_account=true` claim in both callback tokens; MFA is not mandatory. The default legacy policy retains its stronger requirement; a rolled-back signup leaves no grant. The grant primary key is the organization, and repeated inserts, including replacement attempts, do not refill it. The migration gives existing production organizations the same single grant. Simulation organizations receive no real grant and continue to use the separate simulation ledger.
...
For historical whole-cent fixed quotes, confirmed provider acceptance settles the immutable customer price in `estimated_minor`, exactly once. For email/postal delivery quotes, that same acceptance records the frozen `customer_nanoeur` in the fractional ledger; its incremental centime debit is `ceil(new organization total / 10^7) - ceil(previous organization total / 10^7)`. Thus `estimated_minor` is a rounded bound, not a separate one-cent minimum charged for each email. The difference between the reserved ceiling and the actual incremental debit becomes available again.

Fax v3 keeps its full reservation after acceptance or delivery. Only the private verified-usage settlement records the final bounded `customer_nanoeur` in the same cumulative fractional ledger and releases the remaining reservation. The approved cap cannot be exceeded; supplier overrun does not increase the customer's debit. Browser payloads, callback cost claims and Stripe events never determine promotional debits or grants. There is no public price-write or settlement endpoint.

`submission_unknown`, a lost response or an expired processing lease retain the full ceiling. A provider failure before confirmed acceptance also retains it: failed/partial fax transmission can still incur supplier/SIP charges, and a claimed zero cost in a callback is not proof. There is no blind retry or automatic expiry of those holds. A later authenticated acceptance can settle a fixed quote; fax v3 still requires the separate verified-usage proof. Cancellation/preflight failure without any attempt or an explicitly rejected attempt without a supplier reference releases only a still-pending reservation. A failure, complaint or bounce does not automatically refund a settled debit. Contradictory positive evidence for an already released reservation is retained for manual reconciliation rather than silently creating an unfunded send (`credit_reservation_closed`).

Three tenant-scoped journal entry kinds describe reserve, settle and release; each kind can appear only once per dispatch. The credit journal is immutable and its holds, incremental debits and balances remain integer EUR cents. A separate immutable ledger retains nanoEUR consumption for cumulative conversion, including a possible zero incremental centime debit for a positive consumption. Balances never come from a browser-supplied counter. Deleting historical dispatch metadata cannot refund a debit or free an unresolved hold. Organization deletion is not exposed by this feature; preserve the journals in retention/backup procedures.
...
The original strict-policy Auth0 test uses a locally signed JWT fixture with verified email/MFA, confirms the actual signup grant and three disabled channels, then signs in again and proves the same grant remains. Invalid identities create no grant. These are deterministic fixtures; real login, qualified prices and authorized real provider acceptance still require separate evidence. Remote migration proof is recorded in [D1_MIGRATION.md](D1_MIGRATION.md).

Historical initial validation: 18 welcome-credit integration cases and all 272 Vitest tests passed, with typecheck and targeted ESLint passing. `node scripts/migrate-remote.mjs --verify` verified all 14 migrations locally: 113 equivalent schema objects, `quick_check: ok`, no foreign-key violations. It did not contact or change remote D1. Independent review caught and verified the correction for failure arriving before positive provider acceptance; both response and delayed-webhook orderings settled fixed quotes once while preserving the visible failed state.

The published `118ce087` increment passed the integrated 578-test suite and both exact-source CI runs. Fax v3 tests additionally cover retained accepted/delivered/unknown holds, separately verified usage, capped overrun, cumulative fractional debits, replay and conflicting-proof refusal, cross-tenant isolation and immutable financial history. Current test counts, local-versus-hosted scope and publication proof are maintained in [TEST_RESULTS.md](TEST_RESULTS.md) and [LIVE_RELEASE.md](LIVE_RELEASE.md).
