...
`npx vitest run tests/unit/account.test.ts` exercises actual migrations and D1: profile rights/strict schemas, browser-only origin/CSRF controls, pagination/isolation, independent session references, cross-tenant revocation, the distinction between disconnecting and removing membership, last-admin rollback, simultaneous demotions, authority revoked between authentication and commit, and hosted MFA/development-session refusal.
