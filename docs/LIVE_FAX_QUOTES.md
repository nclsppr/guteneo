...
The review page uses the server's `quote_expires_at`, announces the last minute, and disables approval at expiry. A `LIVE_QUOTE_INVALID` rejection also offers renewal while the fax remains prepared and has no attempt. It never increases the quote lifetime or changes an immutable quote.

`POST /api/dispatches/{id}/renew-quote` with an empty JSON body requires the usual authenticated tenant and write authority (`dispatches:prepare` for OAuth). The operation reuses the exact PDF, normalized E.164 number, sender, options and EUR ceiling, then revalidates the current sender, scan, tariff and estimated cost. A changed configuration or insufficient ceiling retains its specific error and leaves the old dispatch unchanged. In one transaction, a successful renewal cancels the old prepared dispatch, creates its replacement and records `fax.quote_renewed`, with no inherited approval or expert review. The transaction requires no provider identity, attempt, outbox or funding reservation on the original, so a concurrent confirmation prevents renewal. No quota, credit or other draft is changed. The old immutable quote and approval history remain available. A replacement is always an individual dispatch, outside any original campaign, even if that campaign is still a draft. The browser announces this before renewal; MCP guidance requires the same disclosure. The frozen manifest, member IDs/fingerprints and other dispatches are untouched; `fax.quote_renewed` records `originalCampaignId`. This also remains safe if the campaign freezes concurrently. The new individual quote requires separate approval.

The server derives a stable renewal key from the original dispatch ID. Concurrent calls and replays return the same replacement, including if that replacement has since been queued; they never create a further fax. If the replacement later expires, renew that new ID. A new browser approval or new expert review under current delegated authority is required for every new quote. A renewal creates no mandate and carries over no review token, fingerprint or consent.

MCP summaries include nullable `quoteExpiresAt` copied directly from the stored quote, including in the tenant-scoped batch projection for `list_dispatches`; clients must never infer it from creation time. `get_dispatch_status` also supplies `attemptCount`. After expiry or `LIVE_QUOTE_INVALID`, the assistant must reread that status and may offer `prepare_fax` with `renewalOf` only for `status=prepared` and explicit `attemptCount=0`; an absent count is unknown, not zero. Repeat the exact `documentId`, E.164 `phone` (including its country code) and integer `ceilingMinor`. The server rejects changed values and retains the original sender/options. Display the returned range, cap and exact new expiry, then obtain fresh approval. Queued, submitting, uncertain, accepted, delivered, failed or attempted dispatches are never recreated through this renewal flow. The backend checks outbox and reservations independently because zero attempts alone does not imply an unsubmitted command.
