...
La réponse REST de préparation est un `Dispatch`, sans `approvalUrl`. Le client ouvre `https://guteneo.com/#/app/dispatch/{id}` avec l’identifiant retourné. MCP construit lui-même `approvalUrl`. Aucun droit OAuth ne crée un consentement humain : `/approve` exige toujours une session navigateur et un contrôle CSRF. Le mode standard utilise cette voie. Les outils MCP experts sont distincts : ils exigent un mandat préalable par connexion OAuth, activé par un administrateur dans Guteneo, puis une revue serveur récente ; leur approbation est déléguée, non humaine. Les scopes seuls ne suffisent pas et les confirmations de l’hôte restent inchangées. Voir [EXPERT_APPROVAL.md](EXPERT_APPROVAL.md). Les routes d’administration, de compte, de facturation, les callbacks et la gestion privée des fournisseurs sont exclus.
