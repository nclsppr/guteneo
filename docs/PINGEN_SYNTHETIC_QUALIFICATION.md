...
Note de connexion (rapport opérateur du 17 septembre 2026) : Wrangler 4.132.0 pouvait rester bloqué pendant la liaison distante. Le parent a qualifié séparément 4.133.0 pour une lecture Telnyx. Le driver permet `run(args, {connect: getPlatformProxy})` avec cette version isolée ; aucune dépendance du produit n’est mise à niveau dans cette tranche. Une connexion réussie avec cette injection ne constitue pas une preuve de dépôt Pingen.
