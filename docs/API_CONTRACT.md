...
Le parcours standard est préparation → consultation du contenu final → approbation humaine → confirmation. Le mandat expert facultatif emploie la revue serveur puis l’acceptation déléguée décrites dans [EXPERT_APPROVAL.md](EXPERT_APPROVAL.md). L’approbation standard est enregistrée en base, liée à l’empreinte de l’envoi et valable au plus 15 minutes ; le parcours expert reste borné par sa revue de cinq minutes et son mandat. **`GET /api/dispatches/:id` renvoie `approval:{fingerprint,expires_at}` uniquement si elle correspond à l’empreinte courante et n’a pas expiré ; sinon `approval:null`.** Le statut reste `prepared` après approbation : le frontend ne doit pas déduire le consentement d’un état local ou du statut seul. Les événements et tentatives renvoyés sont ordonnés et limités à cet envoi.
...
`preparePostalDraft` est une fonction interne du pont Pingen. Le parcours postal contrôle le PDF, puis exige une autorité de transfert distincte : consentement navigateur standard ou mandat postal expert préalable via `transfer_postal_draft`. Aucun de ces dépôts `auto_send:false` n’expédie une lettre. Le devis et l’approbation d’envoi restent des étapes séparées, sous leurs gates effectifs. Voir [Activation réelle](LIVE_ACTIVATION.md).
...
Les résumés MCP utilisent des champs camelCase : document `{id,name,sha256,size,pages,status,source,createdAt,previewUrl,simulation}` ; envoi `{id,channel,status,mode,recipient,documentId,campaignId,fingerprint,estimatedMinor,ceilingMinor,knownMinor,currency,updatedAt,approvalUrl,nextActions}`. `get_dispatch_status` expose ce résumé, pas la timeline complète ni le champ REST `approval`. Le lien d’approbation est `/#/app/dispatch/:id` et exige la connexion humaine. Le mode standard conserve cette approbation navigateur ; `user_confirmed:true` n’est pas accepté. La tranche experte ajoute `review_dispatch`, `approve_and_send_dispatch` et `transfer_postal_draft` sous un mandat préalable, désactivé par défaut, accordé par un administrateur dans le navigateur à une connexion OAuth précise. L’autorité est déléguée et tracée distinctement de la revue humaine. Entrées, scopes, expiration de cinq minutes de la revue et dépôt postal distinct : [EXPERT_APPROVAL.md](EXPERT_APPROVAL.md). Les options MCP actuellement déclarées sont `kind`, `color` et `duplex`, plus étroites que l’objet REST.
