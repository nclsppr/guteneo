...
Le schéma courant redirige l’ancienne URL `/apps-sdk/schemas/` du skill vers `/plugins/schemas/` et exige cette URL canonique dans `$schema`. L’artefact utilise la valeur actuelle. Ses annotations correspondent au candidat corrigé ; publier ce candidat et rescanner les outils avant d’importer puis d’attester ce dossier. Cette génération ne prouve ni compte accessible, ni scan du portail, ni test client réussi.
