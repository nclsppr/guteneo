...
8. Appelez `preflight_postal_pdf` avec `documentId`, `senderId`, le destinataire exact (`name`, `line1`, `postalCode`, `city`, `country`), les options (`deliveryProduct`, `printMode`, `printSpectrum`), `ceilingMinor` en centimes EUR et une `idempotencyKey` stable. Cet outil contrôle le PDF dans Guteneo sans le déposer chez Pingen. Il ne reçoit ni empreinte prétendue, ni rapport fabriqué, ni consentement du modèle. Consultez `get_postal_preflight({preflightId})` et faites ouvrir le `reviewUrl` retourné. Dans Guteneo, la personne doit lire le PDF et l’extrait de fenêtre, puis confirmer la revue et le transfert. Cette préparation fournisseur transmet des données à Pingen même avec `auto_send:false`. Aucun outil MCP ne donne ce consentement à sa place. Un brouillon `prepared` n’est pas encore une lettre analysée ni envoyée ; `unknown` impose de vérifier le transfert, jamais de recréer un contrôle ou une clé pour recommencer. Ne déclenchez aucun preset, redimensionnement ou correction automatique chez Pingen.
9. Après ce transfert consenti, appelez `quote_postal_draft({preflightId, idempotencyKey})`, avec une clé propre à cette demande de devis. Le serveur reprend le même document, destinataire et options. `POSTAL_DRAFT_NOT_READY` signifie que Pingen analyse encore : attendez puis reprenez la même clé, sans nouveau transfert. Présentez le PDF final, l’adresse, les options, le prix client retourné par Guteneo et son plafond. Aucun tarif mémorisé ni calcul de coût à partir d’un prix public ne remplace ce devis. Faites ouvrir le lien d’approbation Guteneo par l’utilisateur. Un « oui » dans le chat ou une permission d’exécuter un outil ne remplace pas l’approbation du manifeste dans sa session. Ne pilotez pas le navigateur pour l’approuver à sa place.
