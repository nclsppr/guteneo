...
ALTER TABLE postal_sender_declarations ADD COLUMN oauth_connection_id TEXT REFERENCES authorized_connections(id);
CREATE TRIGGER validate_postal_sender_submission BEFORE INSERT ON postal_sender_declarations
WHEN (NEW.submission_origin='browser_administrator_declaration' AND (NEW.oauth_client_id IS NOT NULL OR NEW.oauth_connection_id IS NOT NULL))
  OR (NEW.submission_origin='oauth_administrator_submission' AND (NEW.oauth_client_id IS NULL OR length(NEW.oauth_client_id)=0 OR NEW.oauth_connection_id IS NULL OR NOT EXISTS(SELECT 1 FROM authorized_connections c WHERE c.id=NEW.oauth_connection_id AND c.organization_id=NEW.organization_id AND c.user_id=NEW.user_id AND c.client_id=NEW.oauth_client_id AND c.status='active')))
BEGIN
